...
export default function DailyTrafficChart({ data }) {
  ...
  const x = (i) => (n <= 1 ? CHART_W / 2 : PAD_X + (i * (CHART_W - PAD_X * 2)) / (n - 1));
  const y = (value) => PAD_TOP + (1 - value / maxValue) * PLOT_H;

  const viewsPoints = useMemo(() => list.map((d, i) => ({ x: x(i), y: y(d.views || 0) })), [list, maxValue]);
  const visitorsPoints = useMemo(() => list.map((d, i) => ({ x: x(i), y: y(d.visitors || 0) })), [list, maxValue]);

  const areaPath = useMemo(() => {
    if (!viewsPoints.length) return '';
    const baseline = PAD_TOP + PLOT_H;
    return `M${viewsPoints[0].x},${baseline} ` +
      viewsPoints.map((p) => `L${p.x},${p.y}`).join(' ') +
      ` L${viewsPoints[viewsPoints.length - 1].x},${baseline} Z`;
  }, [viewsPoints]);
  ...
  return (
    <div className="traffic-chart">
      ...
      >
        <svg viewBox={`0 0 ${CHART_W} ${CHART_H}`} preserveAspectRatio="none" className="traffic-chart-svg">
          {[0, 0.5, 1].map((frac) => (
            <line
              key={frac}
              x1={PAD_X}
          ...
          <polyline points={toPolyline(viewsPoints)} className="traffic-chart-line traffic-chart-line--views" />
          <polyline points={toPolyline(visitorsPoints)} className="traffic-chart-line traffic-chart-line--visitors" />
      ...
      <div className="traffic-chart-axis">
        {list.map((day, i) => (i % labelEvery === 0 ? (
          <span key={day.date} className="traffic-chart-axis-label" style={{ left: `${(x(i) / CHART_W) * 100}%` }}>
